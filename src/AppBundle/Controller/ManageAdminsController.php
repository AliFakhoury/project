...
class ManageAdminsController extends Controller{
    ...
    public function UpdateEnabled(Request $request, $pageNumber, $id){
        ...
        $EventPending = $em->getRepository('AppBundle:admin')->find($id);
        
        if(!$EventPending){
            return $this->redirectToRoute('ManageAdmins');
        }
        /** @var $post User */
        if($EventPending->getEnabled()== 0){
            $EventPending->setEnabled(1);
        }
        else if($EventPending->getEnabled()==1){
            $EventPending->setEnabled(0);
        }
